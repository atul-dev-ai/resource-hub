...
export default function UserManagementPage() {
  ...
  const handleDeleteUser = async (id: string, role: string) => {
    ...
    try {
      // Calling the custom RPC function we created in SQL
                ...
                <tr key={user.id} className={`transition-colors ${user.role === 'banned' ? 'bg-red-50/50' : 'hover:bg-slate-50'}`}>
                  ...
                  <td className="px-6 py-4">
                    {user.role === 'banned' ? (
                       ...
                       <span className="px-2.5 py-1 rounded-md text-[10px] font-black uppercase bg-indigo-100 text-indigo-700 border border-indigo-200">{user.role.replace('_', ' ')}</span>
                    ) : (
            ...
            >
              <div className="bg-slate-50 border-b border-slate-100 p-6 flex justify-between items-start">
                <div className="flex gap-4 items-center">
                  <div className="relative h-16 w-16 rounded-full bg-emerald-100 border-2 border-white shadow-md flex items-center justify-center overflow-hidden">
                    {selectedUser.avatar_url ? (
                      <Image src={selectedUser.avatar_url} alt="Profile" fill className="object-cover" />
                    ) : (
                      <span className="text-emerald-700 text-xl font-black">{selectedUser.full_name?.charAt(0)}</span>
                    )}
                  </div>
                  <div>
                    <h3 className="text-xl font-black text-slate-900">{selectedUser.full_name}</h3>
